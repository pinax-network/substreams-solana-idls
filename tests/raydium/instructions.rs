use substreams_solana_idls::raydium::amm::v4::instructions::*;
...
#[test]
fn cpmm_unknown() {
    assert!(substreams_solana_idls::raydium::cpmm::instructions::unpack(&[0u8; 16]).is_err());
}
#[test]
fn cpmm_too_short() {
    assert!(substreams_solana_idls::raydium::cpmm::instructions::unpack(&[0u8; 4]).is_err());
}

// --- CLMM V3 ---
#[test]
fn clmm_unknown() {
    assert!(substreams_solana_idls::raydium::clmm::v3::instructions::unpack(&[0u8; 16]).is_err());
}
#[test]
fn clmm_too_short() {
    assert!(substreams_solana_idls::raydium::clmm::v3::instructions::unpack(&[0u8; 4]).is_err());
}
...
fn stable_unknown() {
    ...
    assert!(substreams_solana_idls::raydium::stable::instructions::unpack(&[255u8]).is_err());
}

// --- Launchpad ---
#[test]
fn launchpad_unknown() {
    assert!(substreams_solana_idls::raydium::launchpad::instructions::unpack(&[0u8; 16]).is_err());
}

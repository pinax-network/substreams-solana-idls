#[cfg(test)]
mod tests {
    use base64::prelude::*;
    use substreams::hex;
    use substreams_solana_idls::pancakeswap;

    #[test]
    fn swap_instruction() {
        let bytes = hex!("f8c69e91e17587c8001bb70000000000ffffffffffffffff0000000000000000000000000000000000");
        match substreams_solana_idls::pancakeswap::instructions::unpack(&bytes).expect("decode instruction") {
            substreams_solana_idls::pancakeswap::instructions::PancakeSwapInstruction::Swap(ix) => {
                assert_eq!(
                    ix,
                    substreams_solana_idls::pancakeswap::instructions::SwapInstruction {
                        amount: 12_000_000,
                        other_amount_threshold: u64::MAX,
                        sqrt_price_limit_x64: 0,
                        is_base_input: false,
                    }
                );
            }
            _ => panic!("Expected SwapInstruction"),
        }
    }

    #[test]
    fn swap_event() {
        let bytes = BASE64_STANDARD
            .decode("")
            .expect("base64 decode");

        match substreams_solana_idls::pancakeswap::events::unpack(&bytes).expect("decode event") {
            substreams_solana_idls::pancakeswap::events::PancakeSwapEvent::Swap(event) => {
                assert_eq!(
                    event,
                    substreams_solana_idls::pancakeswap::events::SwapEvent {
                        pool_state: "9HiYSTWK5otBxVc9e9bTNhpRDiDKjki65o9YxRhPWyQC".parse().unwrap(),
                        sender: "3NK19Y8KQyr3HmJVEmn1gjaXDDDpsVVQtBWmHkwZXkAm".parse().unwrap(),
                        token_account_0: "FE4na6rdS6xsCf2WbebozF4tizMW6JXUMX2tdBsyKmsG".parse().unwrap(),
                        token_account_1: "AKaqdf9yFtgArvdXG9LNx6dsR8dC7WAVjRzmJD4gSMUf".parse().unwrap(),
                        amount_0: 2_032_686,
                        transfer_fee_0: 0,
                        amount_1: 12_000_000,
                        transfer_fee_1: 0,
                        zero_for_one: true,
                        sqrt_price_x64: 44_820_234_749_380_015_670,
                        liquidity: 46_810_654_074,
                        tick: 17_756,
                    }
                );
            }
            _ => panic!("Expected SwapEvent"),
        }
    }
}

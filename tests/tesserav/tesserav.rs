...
mod tests {
    use substreams::hex;
    use substreams_solana_idls::tesserav;
    ...
    fn unpack_swap_instruction() {
        ...
        match substreams_solana_idls::tesserav::instructions::unpack(&bytes).expect("decode instruction") {
            substreams_solana_idls::tesserav::instructions::TesseraVInstruction::Swap(ix) => {
                assert!(!ix.is_a_to_b);

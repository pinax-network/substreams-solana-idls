...
mod tests {
    ...
    fn unpack_cpmm_swap_event_v1() {
        ...
        match cpmm::events::unpack(&bytes).expect("decode event") {
    ...
    fn unpack_cpmm_swap_event_v2() {
        ...
        match cpmm::events::unpack(&bytes).expect("decode event") {
            cpmm::events::RaydiumCpmmEvent::SwapEventV2(event) => {
                ...
                assert_eq!(event.input_mint.to_string(), "76rTxzztXjJe7AUaBi7jQ5J61MFgpQgB4Cc934sWbonk", "input_mint",);
                assert_eq!(event.output_mint.to_string(), "So11111111111111111111111111111111111111112", "output_mint",);
            }

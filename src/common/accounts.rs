...
macro_rules! accounts {
    ($name:ident, $getter:ident, { $( $(#[$doc:meta])* $field:ident ),+ $(,)? }) => {
        ...
        impl<'ix> TryFrom<&InstructionView<'ix>> for $name {
            type Error = $crate::common::accounts::AccountsError;
            fn try_from(ix: &InstructionView<'ix>) -> Result<Self, Self::Error> {
                ...
                $(
                    let $field = {
                        ...
                        let a = accounts
                            .get(idx)
                            .ok_or($crate::common::accounts::AccountsError::Missing { name, index: idx })?;
                        let pk = $crate::common::accounts::to_pubkey(name, idx, a.0)?;
                        idx += 1;
                        pk
                    };
                )+
        ...
        pub fn $getter(ix: &InstructionView) -> Result<$name, $crate::common::accounts::AccountsError> {
            $name::try_from(ix)
        }
    };
}
